...
**This skill DISCOVERS and REPORTS. It does NOT apply fixes.** The output is a terse curated report. A separate skill (or a follow-up Pi session) is expected to act on it.
...
- `AGENTS.md` at the repo root, and any nested ones obviously relevant
...
Use the Pi `subagent` tool to run the project `scout` agent with a prompt like:
...
Once `scout` returns, synthesize the candidate list. Apply the hint filter if not already applied. Drop candidates whose hypothesis you don't buy.

If you end up with zero candidates, tell the user honestly and end the skill.

### Checkpoint 1 — User picks areas

Present the candidates in chat and ask the user which areas to investigate. Accept a comma-separated list or `all`.

- Include each candidate's name, hypothesis, path, and evidence.
- Keep the question focused; do not force a fixed number of selections.

If user selects nothing, end the skill politely.

### Phase 3 — Parallel investigation

For each selected area, call the Pi `subagent` tool with the `slop-investigator` project agent.

Run all investigator tasks in one parallel `subagent` call.

Each Agent call:

- `agent`: `"slop-investigator"`
- `agentScope`: `"project"`
- `task`: a short self-contained investigation brief
...
> I've finished discovering candidate areas. The scout returned several evidence-backed candidates. Which areas would you like to investigate?
...
- Do not commit, push, or do any git write operations. (Per project AGENTS.md, always; per skill design, this one is read-only.)

## Portability note

This skill is designed to work in any codebase, not just this one. It does not hardcode project-specific paths or patterns. The investigator reads conventions per repo (`AGENTS.md`, `package.json` / `pyproject.toml` / `go.mod` / `Cargo.toml`, `README.md`) and adapts.

To use in another project:

1. Copy `.pi/skills/find-slop/` and `.pi/agents/slop-investigator.md` into that project's `.pi/` directory.
2. **Restart your Pi session** before the first run. Project agents are discovered at session start; if the session was already open when the files were added, reload or restart Pi before running Phase 3.
3. Run `/find-slop`.
