...
Before writing backend code, read `.pi/docs/backend-conventions.md`.
...
Delegate to the `test-runner` project agent (per AGENTS.md rules).

### 7. Lint check (optional but cheap)

Delegate to the `linter` project agent. Existing MCP tool files may show 4 `TS1005`-style errors from an `ioredis` env issue — that's pre-existing noise and not caused by new tools. Real errors look different (unused imports, mistyped service args, wrong Zod shape).
