...
description: Deep-dive investigator for "AI slop" in one specific area of a codebase. Reads project conventions (AGENTS.md, package.json, README, etc.) first, then hunts for duplication, reinvented wheels, over-engineering, defensive cruft, dead code, comment slop, and performance antipatterns. Returns a terse findings report — or honestly says "no significant findings" if the area is clean. Designed to be spawned in parallel by the /find-slop skill, but can be invoked standalone for a single area.
...
- **Conventions paths**: where to find project conventions (AGENTS.md, package.json, pyproject.toml, README, etc.). Read these BEFORE investigating.
...
- `AGENTS.md` (project root, and any nested ones in the area's parent directories)
...
- Code that **follows** the project's explicit conventions in `AGENTS.md`, even if you'd personally write it differently.
- TypeScript-strictness or tsconfig opinions (different concern).
- "This could be a util" speculation when there's only one usage.
- I18n / locale files (these are blocked from reading in this project per AGENTS.md, and even elsewhere should not be flagged for slop).
